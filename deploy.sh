...
cd dist && zip -r ../public/${APP_NAME}.zip ${APP_NAME}.app && cd ..
...
codesign -dv --verbose=2 ./build/Build/Products/Release/${APP_NAME}.app 2>/dev/null || true

DMG_SIZE=$(ls -lh "public/${DMG_NAME}.dmg" | awk '{print $5}')
ZIP_SIZE=$(ls -lh "public/${APP_NAME}.zip" | awk '{print $5}')
...
echo "   🗜️  public/${APP_NAME}.zip (${ZIP_SIZE}) - Compressed backup version"
echo ""
echo "🌟 DMG Features:"
